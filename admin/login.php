...
if(isset($_POST['user']) && isset($_POST['pass'])){
	...
	if ($verifycode==1 && (!$code || strtolower($code) != $_SESSION['vc_code'])) {
		...
		@header('Content-Type: text/html; charset=UTF-8');
		exit("<script language='javascript'>alert('登陆管理中心成功！释梦QQ：743161828');window.location.href='./';</script>");
	}else {
		unset($_SESSION['vc_code']);
		@header('Content-Type: text/html; charset=UTF-8');
		exit("<script language='javascript'>alert('用户名或密码不正确！释梦QQ：743161828');history.go(-1);</script>");
	}
...
<div id="login-container">
	...
	<small><span id="year-copy"></span> &copy 释梦ye; <a href="#"><?php echo $conf['sitename']?></a></small>
	</footer>
</div>
</body>
</html>